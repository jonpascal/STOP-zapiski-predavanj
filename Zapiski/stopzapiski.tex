...
\begin{definicija}
  ...
  obstaja $\delta > 0$, da velja $d(x, x') < \delta \Rightarrow d'(f(x), f(a)) < \varepsilon$.
...
V metričnih prostorih je $f$ zvezna natanko tedaj, ko velja $\lim_{i \to \infty} f(x_i) = f\left( \lim_{i \to \inf} x_i\right)$.
...
Vsak interval na neskončni osi (vključno z neskončnimi) je homeomorfen enemu izmed intervalov 
$[-1, 1]$, $[-1, 1)$ ali $(-1, 1)$. Intervala $[-1, 1]$ in $(-1, 1)$ nista homeomorfna, saj bi zvezna preslikava 
kompaktno množico (v $\R^n$ zaprt interval) preslika v nekompaktno množico (nezaprt interval). Enako seveda velja tudi 
...
Dokažimo na primer, da je množica $S^{n - 1} \setminus \{(0, \dots, 0, 1)\}$ homeomorfna prostoru $\R^n$.
...
V konkretnih primerih ni težko pokazati, ali je dana preslikava homeomorfna.
...
\begin{zgled}
  ...
  \begin{itemize}
    ...
    Torej sopolinomi gosti v $\topo{C} ([a, b])$ z običajno metriko in zato so tudi polinomi z racionalnimi koeficienti gosto v istem prostoru.
  \end{itemize}
\end{zgled}

\begin{zgled}
  V neskončnem prostoru $X$ s topologijo končnih komplementov je vsaka neskončna podmnožica 
  $A \subseteq X$, saj je $X$ edina zaprta neskončna podmnožica in je torej lahko le $\overline{A} = X$.
\end{zgled}
...
V splošnem pa separabilnost (in celo hkrati separabilnost ter $1$-števnost) ne implicira $1$-števnosti.
...
\begin{zgled}
  ...
  kot tudi v vsakem odprtem krogu $K_2$ s središčem v $b$ nahajajo skoraj vse členi našega zaporedja.
  Če pa polmera krogov $K_1$ in $K_2$ strezno zamnjšamo, da sta disjunktna, pridemo v protislovje.
\end{zgled}
...
\begin{definicija}
  ...
  če obstajata množici $U, V \subseteq \topo{T}$, da je $A \subseteq U$,
...
\begin{dokaz}
  ...
  Tako smo konstruirali zaporedje $(x_n)$, ki konvergira proti $x$ in so vsi $f(x_n) \in A$.
...
ni pa nujno regularna, ker imamo tudi nove zaprte množice, ki se jih ne da ostro ločiti od točk.
...
\begin{zgled}
  ...
  Potem je $\topo{T}'$ Hausdorffova, vendar pa ne moremo ostro ločiti točke $0$,
...
\begin{dokaz}
  Za disjunktni odprti množici $A, B \subseteq X$ definiramo $U = \{x \in X\ |\ d(A, x) < d(B, x)\}$
  in $V = \{x \in X\ |\ d(A, x) > d(B, x) \}$. Očitno je $A \subseteq U$, $B \subseteq V$ in $A \cap B = \emptyset$,
...
\begin{definicija}
  ...
  \begin{itemize}
    \item \textbf{$X$ je $T_0$}: Za različni točki $x,x' \in X$ obstaja okolica ene izmed teh dveh točk, ki jo loči od druge.
    \item \textbf{$X$ je $T_1$}: Za različni točki $x,x' \in X$ obstaja okolica točke $x$, ki jo loči od $x'$ in okolica $x'$, ki jo loči od $x$.
    \item \textbf{$X$ je $T_2$}: Za različni točki $x,x' \in X$ obstajata okolici, ki ostro ločita $x$ in $x'$.
    \item \textbf{$X$ je $T_3$}: Za točko $x\in X$ in zaprto množico $A \subseteq X$, ki ne vsebuje $x$, obstajata okolici, ki ostro ločita $x$ in $A$.
    \item \textbf{$X$ je $T_4$}: Za disjunktni zaprti množici $A; B \subseteq X$ obstajata okolici, ki ostro ločita $A$ in $B$.
  \end{itemize}
\end{definicija}
...
\begin{trditev}
  ...
  odprto okolico $U$ za $A$ obstaja taka zaprta množica $V$, da velja $A \subseteq V \subseteq \overline{V} \subseteq U$.
\end{trditev}
...
\begin{definicija}
  ...
  Obratno je $(X, \topo{T})$ nepovezan, če ni povezan.
\end{definicija}
...
\begin{dokaz}
  ...
  Če obstaja razcep prostora $X$ na neprazni disjunktni odprti množici $X$ 
  in $Y$, potem lahko definiramo $f$, ki slika $U$ v $0$ in $V$ v $1$.
...
\begin{dokaz}
  ...
  Denimo, da lahko poljubni točki v $X$ povežemo s potjo in naj bo $f: X \to \{0, 1\}$ zvezna surjektivna.
...
V splošnem pa komponente niso osprta, kar je jasno že iz primera $\Q$.

Za odprtost komponent mora vsak $x \in X$ imeti okolico $x \in U \subseteq C(x)$.
Zadoščalo bi že, če ima vsak $x$ lokalno bazo iz povezanih okolic.
...
Velja, da so unije s potmi povezanih podmnožic, ki imajo neprazen presek, je povezana s potmi.
...
Vendar pa je razlika v tem, da potne koordinate niso nujno zaprte -- protiprimer je ponovno varšavski lok.
...
\begin{dokaz}
  ...
  Obenem pa je $C'(x)$ komplement unije vseh ostalih potnih koordinat $C(x)$, ki so vse odprte, zato je $C'(x)$ tudi zaprta v $C(x)$.
...
\begin{zgled}
  ...
  \begin{itemize}
    \item Končni primeri so kompaktni.
    \item Če je $(x_n)_{n \in \N}$ konvergentno zaporedje in $\lim_{n \to \infty} x_n = a$,
    potem je $\{x_1, x_2, \dots\} \cup \{a\}$ je kompakt.
...
\begin{dokaz}
  ...
  Naj bo $\mathcal{U} \cup \{\stcomp{A}\}$ je odprto pokritje $X$, zato obstaja končno podpokritje množic tega podpokritja,
  ki je pokritje $A$ z elementi $\mathcal{U}$.
\end{dokaz}
...
\begin{dokaz}
  ...
  Definiramo $U_x = U_1 \cup \dots \cup U_n \ni x$ kot odprto okolico $x$.
  ...
  Ker je $X$ kompaktna, imamo pokritje $\{U_{x_1}, \dots, U_{x_n}\}$ in sedaj je dokaz kompaktnosti na dlani.
\end{dokaz}
...
\begin{dokaz}
  Za vsak $a \in K$ in $x \notin A$ obstajata odprti množici $U_a \ni a$ in $V_a \ni x$, da je $U_a \cap V_a = \emptyset$.
  Ker družina množic $\{U_a\}$ pokriva $A$, obstaja podpokritje $U_{a_1} \cup \dots \cup U_{a_n} \supseteq K$.
...
\begin{dokaz}
  ...
  Denimo, da $A$ nima stekališč. Potem za vsak $x \in X$ obstaja $U_x$, ki vsebuje le končno mnogo 
...
\begin{dokaz}
  ...
  $U_{a_1} \cup \dots \cup U_{a_n} \supset A$ in $V_{a_1} \cap \dots \cap V_{a_n} \supseteq B$ odprti disjunktni množici, ki ločita $A$ in $B$.
\end{dokaz}
...
\begin{dokaz}
  ...
  $U_1, \dots, U_n$ pokritje. Sedaj definiramo $\lambda = \min_{x \in K} f(x) > 0$.
...
\begin{dokaz}
  ...
  $K$ kompakt, je $\overline{V}$ zaprta in zato prav tako kompaktna.
\end{dokaz}
...
\begin{dokaz}
  Dokazati moramo, da nobeni odprta podmnožica $X$ ni vsebovana v $\bigcap_{i = 1} ^\infty F_i.$
  Naj bo $U_0 \subseteq X$ odprta.
  Ker je $\inte\, F_1 = \emptyset$, $U_0$ ni podmnožica $F_1$, zato obstaja $x_1 \in U_0 \setminus F_1$.
  Potem obstaja relativno kompaktna okolica $x \in U_1 \subseteq U_0$, ki ne seka $F_1$.
  ...
  To počnemo v neskončnost in dobimo množice $$\overline{U_1} \supset \overline{U_2} \supset \dots \bigcap \overline{U}_i \neq \emptyset,$$
  torej obstaja $x \in U_0 \setminus \bigcup_1 ^\infty F_i$.
\end{dokaz}
...
\begin{zgled}
  ...
  supremum metriko, zato je dovolj pokazati, da so množice $N_k$ zaprte in s prazno notranjostjo (to lahko pokažemo s preslikavo lomljenko).
  Od tod po Bairu sledi, da nikjer odvedljive zvezne preslikaveobstajajo in so goste v prostoru vseh preslikav $\mathcal{C} (I)$.
\end{zgled}
...
\begin{izrek}
  $(X^+, \mathcal{T})$ je kompakten prostor. Če $X$ ni kompakten, je inkluzija $i: X \hookrightarrow X^+$ kompaktifikacija, ki je pravimo 
...
potem za $x \neq x'$ v $X$ lahko najdemo $B, B' \subseteq \topo{B}$,
...
Če je $\mathcal{B}$ torej števna, potem obstaja števna družina funkcij v $C(X, [0, 1])$, ki loči točke $X$.
...
\begin{dokaz}
  ...
  Obstajata bazični okolici $B$ in $B'$, za kateri velja $x \in X \subseteq \overline{X} \subseteq X'$
...
\begin{lema}
  ...
  za katero je $|f(a) - h(a)| \leq \frac{2}{3} c$.
\end{lema}
...
Očitno je produkt absolutnih ekstenzorjev.
...
\begin{lema}
  Funkcijo $\varphi(t) = \sqrt{t}$ je na $[0, 1]$ enakomerna limita polinomov.
\end{lema}
...
\begin{dokaz}[Dokaz izreka]
  ...
  Torej imamo končno podpokritje $U_{v_1}, \dots, U_{v_n}$ in sedaj definiramo 
  $h_u (x) = \min \{h_{u, v_1} (x), \dots, h_{u, v_n} \}$.
